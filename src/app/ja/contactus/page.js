...
export default function ContactForm() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name) {
      ...
      newErrors.name = "氏名に数字のみは使用できません";
    }
    ...
    if (
      ...
      if (formData.phoneNumber1 && !/^\d{3}$/.test(formData.phoneNumber1)) {
        newErrors.phoneNumber1 = "3桁の数字を入力してください";
      }
    ...
    if (!formData.emailAddress) {
      ...
      newErrors.emailAddress = "正しいメールアドレス形式で入力してください";
    }
    ...
    if (currentStep === 1) {
      ...
      try {
        ...
        if (data.success) {
          ...
          alert(
            `エラー: ${data.message || "お問い合わせの送信に失敗しました。"}`
          );
          setCurrentStep(1);
        }
